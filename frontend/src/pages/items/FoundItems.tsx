...
import {Loader2,Search,PackageCheck,ShieldCheck,MessageCircle} from "lucide-react";
...
import {getFoundItems,getItemsByCategory,searchItems} from "@/services/itemService";
...
export default function FoundItems(){
 ...
 const loadItems=async()=>{
  try{
   setLoading(true);
   const data=search.trim()?await searchItems(search):
    category!=="ALL"?await getItemsByCategory(category as any):
    await getFoundItems();
 ...
 useEffect(()=>{loadItems()},[search,category]);

 const features=[
  [<PackageCheck className="text-orange-500"/>,"Verified Reports","Safe item recovery process"],
  [<ShieldCheck className="text-orange-500"/>,"Secure Claims","Protected ownership verification"],
  [<MessageCircle className="text-orange-500"/>,"Private Chat","Coordinate handover safely"]
 ];

 return <div className="mx-auto max-w-7xl space-y-10 px-6 py-10">
   ...
   <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
    <div>
     ...
     <p className="mt-3 max-w-xl text-lg text-orange-100">
      Discover items reported by employees and help return them to their rightful owners.
     </p>
    </div>
