...
const Navbar = () => {
  ...
  const navClass = (p: string) => active(p) ? "font-semibold text-orange-500" : "text-slate-600 transition hover:text-orange-500";

  const logout = () => { localStorage.clear(); navigate("/") };

  const menu = (items: [string, string][]) => items.map(([to, text]) =>
    <Link key={to} to={to} className="block rounded-lg px-3 py-2 text-sm text-slate-700 transition hover:bg-orange-50 hover:text-orange-500">
      {text}
    </Link>
  );

  const Drop = ({ title, items }: { title: string; items: [string, string][] }) =>
    <div className="relative group">
      <button className="flex items-center gap-1 text-slate-600 transition hover:text-orange-500">
        {title}<ChevronDown size={16} />
      </button>

      <div className="invisible absolute left-0 top-8 w-52 rounded-xl border bg-white p-2 opacity-0 shadow-lg transition-all duration-200 group-hover:visible group-hover:opacity-100">
        {menu(items)}
      </div>
    </div>;


  return <header className="sticky top-0 z-50 border-b bg-white shadow-sm">
        ...
        {token ?
          <>
            ...
            <Button
              variant="ghost"
              size="icon"
              className="relative h-11 w-11 rounded-full bg-orange-50 hover:bg-orange-100"
